...
print('lwage ~ educ + age + age2 + age3 + age4 + female + white')
print(lm.summary())

# R squared is essentially the same as before adding in extra age terms. Fit not improved.
...
print('hrwage ~ educ8+educ9+educ10+educ11+educ12+educ13+educ14+educ15+educ16+educ17+educ18')
print(lm.summary())

lm = smf.ols(formula='lwage ~ educ8+educ9+educ10+educ11+educ12+educ13+educ14+educ15+educ16+educ17+educ18', data = df).fit()
print('LWAGE ~ educ8+educ9+educ10+educ11+educ12+educ13+educ14+educ15+educ16+educ17+educ18')
print(lm.summary())

""" using C() makes dummy for each VALUE of educ, not each year
lm = smf.ols(formula='lwage ~ C(educ)', data = df).fit()
print(lm.summary())
"""

# The effects of education are nonlinear in both the hrwage and lwage models. The 16th year of education (ie graduating from college) has a disproportionately positive effect on wages in both cases. 
...
print('LWAGE ~ educ8+educ9+educ10+educ11+educ12+educ13+educ14+educ15+educ16+educ17+educ18+ CONTROLS')
print(lm.summary())

# The fit of the model has improved substantially (r-squared increased to .96)
...
# There is evidence of possible heteroskedacity in both hrwage and lwage models with larger variation in wages as educ increases
...
print("p-value of the f-statistic of the hypothesis that the error variance does not depend on x", white)

# White standard errors are slightly larger for intercept, educ, age, and age2. They are slightly smaller for female and white. The white test shows evidence of heteroskedasticity. The p-value of the f-statistic of the hypothesis that the error variance does not depend on x is .0004.
...
print('res_lwage2 ~ educ + age + age2 + female + white')
print(lm.summary())
...
print('res_hrwage2 ~ educ + age + age2 + female + white')
print(lm.summary())
...
# Residuals from hrwage model appear more heteroskedastic (r-squared of .05 vs r-squared of .025 for lwage model) ie the regressors from the hrwage model are better predictors of that model's residuals than the lwage model's regressors are of that model's residuals.
...
print('res_lwage2 ~ educ + age + age2 + female + white + educAge + femAge + femEduc + whiteAge + whiteEduc')
print(lm.summary())

# hrwage
lm = smf.ols(formula='res_hrwage2 ~ educ + age + age2 + female + white + educAge + femAge + femEduc + whiteAge + whiteEduc', data = df).fit()
print('res_hrwage2 ~ educ + age + age2 + female + white + educAge + femAge + femEduc + whiteAge + whiteEduc')
print(lm.summary())

# Including these interaction variables helps explain more of the variation in the residuals of both models, ie the residuals appear more heteroskedastic when tested with these variables. Adjusted r-squares nearly doubled for both models.
...
# Twins are probably correlated with one another across a number of characteristics, e.g. ability, family upbringing, etc. If one twin has a positive residual, we could guess that the other might, as well.
...
print('USING CLUSTERS, lwage ~ educ + age + age2 + female + white')
print(lm_C.summary())

# SE is bigger on educ because the intracluster correlations are positive. Without accounting for clustering, we effectively overestimate our sample size. 

#TODO: cluster argument not working
...
# Yes, this effectively corrects for the clustering problem. Collapsing to group means is an extreme version of using clustered standard errors--we've reduced our number of observations down to the number of groups.
...
"""
# The expected value of our estimator will be biased downwards (1-(d/(1-r))) where d is the noise to total variance ratio (var of measurement error / var of observed lwages) and r is the r-squared value from our regression of the the X's on observed lwage.
...
print('USING IV, lwage ~ educ_pred + age + age2 + female + white')
print(lm_IV.summary())
#NOTE TODO: .fit() method arguments not working. Standard errors unchanged.

print(lm_lw.summary())


"""
lm3 = smf.ols(formula = 'lwage ~ educt_t + age + age2 + female + white', data=df).fit()
print('USING EDUCT_T DIRECTLY, lwage ~ EDUCT_T + age + age2 + female + white')
print(lm2.summary())
"""

print("AVG EDUC")
print(np.mean(df.educ))
print("AVG EDUCT_T")
print(np.mean(df.educt_t))

# Return on education is slightly higher because the twin-estimated education values are slightly smaller than self-estimated values (avg 13.96 vs 14.03), giving fewer years of education the same amount of credit.
...
# The fact that our estimated returns to educ halved when holding (ostensibly) all omitted variables constant shows that our previous results were probably biased upwards as predicted.
...
lm_IV = smf.ols(formula = 'dlwage ~ deduc_pred', data=df).fit()
print('USING IV, dlwage ~ deduc_pred')
print(lm_IV.summary())
...
# Each year of education increases wages by 7%. Each year of experience increases wages by 3% until a certain pt, after which wages flatten or fall. Being female reduces wages by 22%. Being married increases wages by 4%. Using a computer at work increases wages by 17%.
...
# Returns to ed reduced to 6%, returns to computer reduced to 12%. Results significantly different. 
